...
const DisplayBlog: React.FC<Props> = (props) => {
    ...
    return (
        <div className="blog-class-main">
            <div className={showMore ? "display-blog-expanded" : "display-blog-collapsed"}>
                {parser(blogData)}
            </div>
            <hr></hr>
            <div className="show-more-button" onClick={() => setShowMore(!showMore)}>{showMore ? "Collapse" : "Expand" }</div>
        </div>
    )
