...
const DisplayBlog: React.FC<Props> = (props) => {
    ...
    return (
        <div className={`w-8/12 m-auto ${theme.bgFifth} rounded-lg my-8 shadow-2xl ${theme.textFirst} text-lg`}>
            <div className={showMore ? `p-4` : "p-4 overflow-hidden max-h-52"}>
                {parser(blogData)}
            </div>
            <div className={`flex justify-center mt-2 p-2 font-bold ${theme.textFifth} ${theme.bgFirst}`} onClick={() => setShowMore(!showMore)}>{showMore ? "Collapse" : "Expand" }</div>
        </div>
    )
